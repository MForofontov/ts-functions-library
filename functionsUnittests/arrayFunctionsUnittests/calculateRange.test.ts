import { calculateRange } from '../../arrayFunctions/calculateRange';
...
describe('calculateRange', () => {
    ...
    it('8. should handle arrays with large numbers', () => {
        expect(calculateRange([1000000000, 2000000000, 3000000000])).toBe(2000000000);
        expect(calculateRange([-1000000000, -2000000000, -3000000000])).toBe(2000000000);
    });

    // Test case 9: Range of an array with very small numbers
    it('9. should handle arrays with very small numbers', () => {
        expect(calculateRange([0.0000001, 0.0000002, 0.0000003])).toBeCloseTo(0.0000002);
        expect(calculateRange([-0.0000001, -0.0000002, -0.0000003])).toBeCloseTo(0.0000002);
    });
