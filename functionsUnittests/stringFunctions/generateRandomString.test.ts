...
describe('generateRandomString', () => {
    ...
    it('5. should throw an error when length is non-numeric', () => {
        const length: any = "a";
        const charset: string = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
        expect(() => generateRandomString(length, charset)).toThrow('Length must be a non-negative number');
    });
    ...
    it('7. should throw an error when charset is empty', () => {
        ...
        expect(() => generateRandomString(length, charset)).toThrow('Charset must contain at least one character');
    });
    ...
    it('9. should generate a random string with special characters in charset', () => {
        ...
        const result: string = generateRandomString(length, charset);
        expect(result).toHaveLength(length);
        expect(result).toMatch(new RegExp(`^[${charset.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&')}]+$`));
    });
