...
describe('getISOWeekDate', () => {
    ...
    it('5. should return the ISO week date for a date at the end of the year', () => {
        ...
        const expected: string = '2024-W01-1';
        const result: string = getISOWeekDate(date);
        expect(result).toBe(expected);
    });
    ...
    // Error test case 1: Get the ISO week date for a NaN date (should throw an error)
    it('9. should throw an error for a NaN date', () => {
        const date: Date = new Date(NaN);
        expect(() => getISOWeekDate(date)).toThrow('Invalid date');
    });

    // Error test case 2: Get the ISO week date for an invalid date (should throw an error)
