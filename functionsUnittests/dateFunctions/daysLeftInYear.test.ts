...
describe('daysLeftInYear', () => {
    ...
    // Test case 6: Get the number of days left in the year for a NaN date (should throw an error)
    it('6. should throw an error for a NaN date', () => {
        const date: Date = new Date(NaN);
        expect(() => daysLeftInYear(date)).toThrow('Invalid date');
    });

    // Test case 7: Get the number of days left in the year for an invalid date (should throw an error)
