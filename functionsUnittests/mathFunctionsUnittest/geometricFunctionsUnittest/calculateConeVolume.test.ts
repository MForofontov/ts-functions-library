...
describe('calculateConeVolume', () => {
    ...
    it('6. should throw an error for negative radius', () => {
        const radius: number = -5;
        const height: number = 10;
        expect(() => calculateConeVolume(radius, height)).toThrow('Radius must be a non-negative number');
    });
    ...
    it('7. should throw an error for negative height', () => {
        const radius: number = 5;
        const height: number = -10;
        expect(() => calculateConeVolume(radius, height)).toThrow('Height must be a non-negative number');
    });
