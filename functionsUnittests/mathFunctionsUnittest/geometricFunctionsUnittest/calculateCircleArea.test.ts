...
describe('calculateCircleArea', () => {
    ...
    it('6. should throw an error for a negative radius', () => {
        const radius: number = -5;
        expect(() => calculateCircleArea(radius)).toThrow('Radius must be a non-negative number');
    });
    ...
    it('7. should throw an error for NaN radius', () => {
        ...
        expect(() => calculateCircleArea(radius)).toThrow('Radius must be a number');
    });

    // Test case 8: Area of a circle with a string radius (should throw an error)
