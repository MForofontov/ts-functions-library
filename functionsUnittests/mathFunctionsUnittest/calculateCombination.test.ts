...
describe('calculateCombination', () => {
    ...
    it('7. should throw an error for floating-point inputs', () => {
        const n: number = 5.5;
        const k: number = 3;
        expect(() => calculateCombination(n, k)).toThrow('Both n and k must be integers');
    });
    ...
    it('8. should throw an error for negative inputs', () => {
        const n: number = -5;
        const k: number = 3;
        expect(() => calculateCombination(n, k)).toThrow('Both n and k must be non-negative integers');
    });

    // Test case 9: Combination with NaN inputs
    it('9. should throw an error for NaN inputs', () => {
        expect(() => calculateCombination(NaN, 3)).toThrow('Both n and k must be numbers');
        expect(() => calculateCombination(5, NaN)).toThrow('Both n and k must be numbers');
    });
});
